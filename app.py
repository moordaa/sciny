...
st.set_page_config(page_title="Ściny Web v1.5", page_icon="🪵", layout="centered")
...
if not st.session_state.zalogowany:
            ...
            with st.form("form_wydania", clear_on_submit=True):
                ...
                if st.form_submit_button("ZAPISZ WYDANIE", type="primary", use_container_width=True):
                    ...
                    supabase.table("wydania_scin").insert({
                        "pracownik_id": p_id, "data": str(data_wyb), "dlugosc": dlu,
                        "obstawki": obs, "m3": m3, "adnotacja": uwagi,
    ...
    # --- ZAKŁADKA PRACOWNICY ---
        ...
        st.title("👥 Pracownicy")
        nowy = st.text_input("Dodaj nazwisko pracownika")
        if st.button("DODAJ", type="primary") and nowy:
            try:
        ...
        if pracownicy:
            for p in pracownicy:
                if isinstance(p, dict):
                    ...
                    c1.write(f"👷 {p.get('nazwa')}")
                    if c2.button("Usuń", key=f"p_{p.get('id')}"):
    ...
    # --- WYSZUKIWARKA I STATYSTYKI ---
        ...
        st.title("🔐 Konta")
        with st.form("new_u"):
            ul, up = st.text_input("Login"), st.text_input("Hasło", type="password")
