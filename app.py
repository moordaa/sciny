...
def pobierz_pracownikow():
    try:
        ...
        # Zabezpieczenie przed TypeError: zawsze zwraca listę
...
if wybor == "📝 Nowe Wydanie":
    ...
    if not pracownicy:
        ...
        # Naprawa TypeError przy braku klucza 'nazwa'
...
# --- PRACOWNICY ---
    ...
    with st.form("f_dodaj", clear_on_submit=True):
        ...
        if st.form_submit_button("DODAJ PRACOWNIKA", use_container_width=True):
            if nowy:
                try:
                    # Dodawanie z weryfikacją odpowiedzi z bazy
                    res_ins = supabase.table("sciny_pracownicy").insert({"nazwa": nowy}).execute()
                    if res_ins.data:
                        st.success(f"Dodano: {nowy}")
                        st.rerun()
                    else:
                        st.error("Baza nie zwróciła potwierdzenia. Sprawdź SQL Editor.")
                except Exception as e:
    ...
    if not pracownicy:
        ...
        for p in pracownicy:
            if isinstance(p, dict) and 'nazwa' in p and 'id' in p:
