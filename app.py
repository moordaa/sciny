    ...
    with st.container(border=True):
        ...
        if st.button("ZALOGUJ", use_container_width=True, type="primary"):
            if l == "Emil" and p == "Sosna100%":
                ...
                except: st.error("Błąd połączenia z bazą kont.")
    st.stop()
...
with st.sidebar:
    ...
    wybor = st.radio("Nawigacja", ["📝 Nowe Wydanie", "🔍 Historia", "📈 Statystyki", "👥 Pracownicy", "📊 Eksport"])
    st.divider()
...
if wybor == "👥 Pracownicy":
    ...
    with st.form("f_nowy_p", clear_on_submit=True):
        ...
        if st.form_submit_button("DODAJ PRACOWNIKA", type="primary"):
            if nowy_p:
                res = supabase.table("sciny_pracownicy").insert({"nazwa": nowy_p}).execute()
                if res.data:
                    st.success(f"Dodano: {nowy_p}")
                    st.rerun()
                else:
                    st.error("Baza nie potwierdziła zapisu. Sprawdź SQL Editor.")
            else: st.warning("Wpisz imię!")

    st.divider()
    lista = pobierz_pracownikow()
    if not lista:
        st.info("Brak pracowników. Dodaj pierwszą osobę powyżej.")
    for p in lista:
    ...
    if not pracownicy:
        st.warning("Dodaj pracowników w zakładce 'Pracownicy'.")
    else:
